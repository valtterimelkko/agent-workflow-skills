#!/usr/bin/env python3
"""
Pain point detection, sentiment analysis, and opportunity scoring algorithms for Twitter.

Provides comprehensive text analysis for identifying SaaS opportunities from Twitter content.
"""
import re
...
from scripts.utils import (
    ScrapedTweet, ScrapedReply, VelocityMetrics, 
    CompetitorMention, OpportunitySignal, parse_tweet_date
)
...
class OpportunityAnalyzer:
    """Analyzes Twitter content for SaaS opportunities."""
    
    def __init__(self):
        self.high_intent_patterns = HIGH_INTENT_PATTERNS
        self.frustration_keywords = FRUSTRATION_KEYWORDS
        
    def analyze_tweet(self, tweet: ScrapedTweet) -> Dict[str, Any]:
        """
    ...
    def calculate_velocity_metrics(self, tweet: ScrapedTweet) -> VelocityMetrics:
        """Calculate engagement velocity for a tweet."""
        try:
            created_utc = parse_tweet_date(tweet.created_at)
        ...
        likes_per_hour = tweet.engagement / hours_since_post
        replies_per_hour = tweet.reply_count / hours_since_post
        retweets_per_hour = tweet.retweet_count / hours_since_post
        
        # High reply-to-like ratio often indicates controversy/pain
        engagement_ratio = tweet.reply_count / max(1, tweet.engagement)
        
        # Simple acceleration estimation (would need historical data for true calc)
        acceleration = 0.0
        
        # Viral probability based on engagement rate
        viral_probability = min(100, (likes_per_hour * 10) + (replies_per_hour * 20))
        
        return VelocityMetrics(
            likes_per_hour=round(likes_per_hour, 2),
            replies_per_hour=round(replies_per_hour, 2),
            retweets_per_hour=round(retweets_per_hour, 2),
    ...
    def analyze_reply_thread(self, replies: List[ScrapedReply]) -> Dict[str, Any]:
        """Analyze reply thread for agreement indicators and pain amplification."""
        total_agreements = 0
        total_sentiment = 0
        reply_count = 0
        
        def traverse(replies_list: List[ScrapedReply], depth: int = 0):
            nonlocal total_agreements, total_sentiment, reply_count
            ...
            for reply in replies_list:
                ...
                for pattern in AGREEMENT_PATTERNS:
                    if re.search(pattern, text_lower):
                        total_agreements += 1
                        reply.agreement_indicators += 1
                
                # Calculate sentiment
                sentiment = self._calculate_frustration_score(text_lower) / 100
                reply.sentiment_score = sentiment
                total_sentiment += sentiment
                
                # Traverse nested replies
                if reply.replies:
                    traverse(reply.replies, depth + 1)
        
        traverse(replies)
        
        avg_sentiment = total_sentiment / max(1, reply_count)
        agreement_ratio = total_agreements / max(1, reply_count)
    ...
    def synthesize_opportunity(
        ...
        return OpportunitySignal(
            ...
            target_audience=tweet.metadata.get("hashtags", ["unknown"])[0] if tweet.metadata.get("hashtags") else "unknown",
